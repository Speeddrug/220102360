...
export default function ShortenerPage() {
  ...
  useEffect(() => { Log("frontend", "info", "page", "ShortenerPage loaded"); }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      for (let i = 0; i < rows.length; i++) {
        ...
        if (isNaN(validityInt) || validityInt <= 0) throw new Error(`Row ${i + 1}: Validity must be positive integer`);
        if (shortcode && !validateShortcode(shortcode)) throw new Error(`Row ${i + 1}: Shortcode invalid`);

        const linkObj = storageService.createShortLink({ longUrl, validityMinutes: validityInt, preferredShortcode: shortcode || null });
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Typography variant="h5" gutterBottom sx={{ mb: 3, fontWeight: 'bold' }}>Shorten a long URL</Typography>
          ...
          <form onSubmit={handleSubmit}>
            {rows.map((r, idx) => (
              <Box key={idx} sx={{ mb: 2 }}>
                <TextField
                  ...
                  sx={{ mb: 1 }}
                />
                <Typography variant="body2" sx={{ mb: 1 }}>Customize your link (optional)</Typography>
            ...
            <Button variant="contained" color="success" fullWidth sx={{ mt: 2 }} type="submit">Shorten URL</Button>
            <Button variant="outlined" fullWidth sx={{ mt: 1 }} onClick={addRow}>Add another URL</Button>
          </form>

          {results.length > 0 && (
            <Box sx={{ mt: 4 }}>
              <Typography variant="h6" sx={{ mb: 2 }}>Created Links</Typography>
              {results.map(r => (
                <Paper key={r.shortcode} sx={{ p: 2, my: 1, backgroundColor: 'rgba(0,0,0,0.05)' }}>
                  <Typography>
                    <strong>Short URL:</strong> <a href={`https://${DOMAIN}/${r.shortcode}`} target="_blank" rel="noopener noreferrer">
                      {DOMAIN}/{r.shortcode}
                    </a>
                  </Typography>
